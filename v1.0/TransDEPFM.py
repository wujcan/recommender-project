...
class TransDEPFM(object):
    ...
    def _create_ego_interaction_graph_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            # all_embeddings += [ego_embeddings]
        all_embeddings = tf.concat(ego_embeddings, 1)
        return all_embeddings
    ...
    def _create_transfm_predictions(self, feats):
        ...
        # Common terms
        var_factor_product = tf.reduce_sum(tf.square(self.var_a_factor), axis=1, keepdims=True)
        var_trans_product = tf.reduce_sum(tf.square(self.weights['var_trans']), axis=1, keepdims=True)
        var_factor_trans_product = tf.reduce_sum(tf.multiply(self.var_a_factor, self.weights['var_trans']),
                                                 axis=1, keepdims=True)

        # Common terms
        feats_sum = tf.sparse_reduce_sum(feats, axis=1, keep_dims=True)
        factor_mul = tf.sparse_tensor_dense_matmul(feats, self.var_a_factor)
