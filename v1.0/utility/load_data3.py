...
class Data(object):
    def __init__(self, path):
        ...
        # test_neg_file = path + '/test_neg.data'
    ...
    def get_interaction_mat(self, train_file, valid_file, test_file):
        ...
        def get_sp_mat(file_name):
            ...
            with open(file_name) as f:
                ...
                sp_feat_mat = sp.coo_matrix((fvals, (frows, fcols)), shape=(self.n_users, self.n_feats))
    ...
    def generate_sp_train_batch(self, batch_size):
        ...
        # feat = self.feat_one_hot[feat_idx]
        tmp = np.array(self.train_label)
        feat_to_csr = self.train_feats.tocsr()
        labels = tmp[label_idx]
        labels = labels[:, 1]
        feat = []
        ...
        # labels = self.train_label[label_idx]
