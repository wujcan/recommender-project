...
class FM(object):
    ...
    def _create_log_loss(self, sp_feats):
        ...
        # log_loss = tf.contrib.losses.log_loss(tf.reshape(self.preds, [-1]),
        #                                       tf.reshape(self.sp_labels, [-1]))

        # log_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
        #     logits=tf.reshape(self.preds, [-1]),
        #     labels=tf.reshape(self.sp_labels, [-1])))
